...
describe EnqueueUpdatePatientsFromPDSJob do
  ...
  it "only queues jobs for the appropriate patients" do
    expect { perform }.to enqueue_sidekiq_job(PatientUpdateFromPDSJob).exactly(
      4
    ).times
  end

  it "queues a job for the invalidated patient" do
    expect { perform }.to enqueue_sidekiq_job(PatientUpdateFromPDSJob).with(
      invalidated_patient.id,
      nil
    )
  end

  it "doesn't queue a job for the deceased patient" do
    expect { perform }.not_to enqueue_sidekiq_job(PatientUpdateFromPDSJob).with(
      deceased_patient.id,
      nil
    )
  end
  ...
  it "queues a job for the not recently updated patient" do
    expect { perform }.to enqueue_sidekiq_job(PatientUpdateFromPDSJob).with(
      not_recently_updated_patient.id,
      nil
    )
  end
