...
describe EnqueueVaccinationsSearchInNHSJob, :within_academic_year do
  ...
  let(:session) do
    send_consent_requests_at =
      (location.generic_clinic? ? nil : send_consent_requests_or_invitations_at)
  ...
  describe "doing the rolling searches" do
    ...
    let(:school) { create(:gias_school, team:, programmes:, gias_year_groups:) }
    let(:location) { school }
    ...
    context "with many patients" do
      ...
      it "searches for 1/28th of the elligible patients" do
        ...
        expect(SearchVaccinationRecordsInNHSJob).to have_received(
          :perform_bulk
        ).once.with(match([anything, anything]))
      end
      ...
      context "when most of the patients have had a search done recently" do
        ...
        it "searches for 1/28th of the elligible patients" do
          ...
          expect(SearchVaccinationRecordsInNHSJob).to have_received(
            ...
            expect(patients_with_older_searches.map(&:id).zip).to include(*args)
          end
