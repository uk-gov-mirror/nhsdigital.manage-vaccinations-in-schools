...
describe Reports::OfflineExporter do
  ...
  shared_examples "generates a report" do
    ...
    describe "#from_patients" do
      subject(:workbook) do
        RubyXL::Parser.parse_buffer(
          described_class.from_patients(
            Patient.all,
            team:,
            ...
            expect(rows.first.except("PERSON_DOB")).to eq(
              {
                ...
                "REASON_NOT_VACCINATED" => "",
                "SCHOOL_NAME" => patient.school&.name,
                "SESSION_ID" => "clinic",
                "SUPPLIER_EMAIL" => "",
                "TIME_OF_VACCINATION" => "",
                "TRIAGED_BY" => nil,
    ...
    describe "#from_session" do
      subject(:workbook) do
        RubyXL::Parser.parse_buffer(described_class.from_session(session))
      end
        ...
        describe "rows" do
          ...
          context "with a vaccinated patient" do
            ...
            it "adds a row with the vaccination details" do
              ...
              expect(rows.first["BATCH_EXPIRY_DATE"].to_date).to eq(
                batch_expiry
              )
              expect(rows.first["PERSON_DOB"].to_date).to eq(
                patient.date_of_birth
          ...
          context "with a vaccinated patient for a different programme" do
            before do
              ...
              other_programme_type =
                (Programme::TYPES - [programme.type]).sample
